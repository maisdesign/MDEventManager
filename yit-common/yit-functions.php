...
} //!function_exists('yit_is_woocommerce_active')
...
} //!function_exists('yit_get_plugin_basename_from_slug')
    ...
    function yith_debug()
    {
        ...
        if (!empty($args)) {
            foreach ($args as $k => $arg) {
                ...
                $output = '<pre class="yit-debug">' . '<strong>$param_' . ($k + 1) . ": </strong>" . $output . '</pre>';
                echo $output;
            } //$args as $k => $arg
        } //!empty($args)
        else {
...
} //!function_exists('yit_get_options_from_prefix')
if (!function_exists('yit_wp_roles')) {
    ...
    function yit_wp_roles()
    {
        ...
        foreach ($wp_roles->roles as $k => $role) {
            $roles[$k] = $role['name'];
        } //$wp_roles->roles as $k => $role
        return $roles;
    }
} //!function_exists('yit_wp_roles')
...
} //!function_exists('yit_user_roles')
// ADMIN
